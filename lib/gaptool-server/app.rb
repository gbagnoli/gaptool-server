...
#      out = hosts.merge!(host.gsub(/host:/, '').gsub(/:/,'-') => Resolv.getaddress(@redis.hget(host, 'hostname')))
#      if @redis.hget(host, 'alias')
#        out = hosts.merge!(@redis.hget(host, 'alias') => Resolv.getaddress(@redis.hget(host, 'hostname')))
#      end
#    end
#    puts hosts
#
